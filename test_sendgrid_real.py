#!/usr/bin/env python
"""
Test real SendGrid email sending with API key
"""
import os
import django
...
print(f"API Key: {os.environ.get('SENDGRID_API_KEY', 'NOT SET')[:20]}...")
print()
...
items = [
    ...
    {'serial_number': '20110106100105', 'component_type': 'Capacitor', 'description': '100µF 25V', 'quantity_received': 30},
]
...
# Send test email with real SendGrid
try:
    ...
    recipient_list = ['snehal@deyeindia.com']  # Test with one recipient first
    ...
    result = msg.send(fail_silently=False)  # Use fail_silently=False to see errors
    
    if result > 0:
        print(f"✅ SUCCESS! Email sent to {result} recipient(s)")
        print()
